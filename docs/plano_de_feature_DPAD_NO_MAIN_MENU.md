...
- [ ] Deshook total do MainMenu (estado arquitetural correto, molde Mail/Merchant): remover `ConsoleModeMainMenuFrame` da whitelist `Hooks.frames:18-97` e do caminho espacial (`FindFirstVisibleButton:493-608`, `CollectButtons:664-702`); `MainMenuNav` assume ciclo próprio `EnterNavigationMode` no `Show` / `ExitNavigationMode` no `Hide`, e a pilha de B/fechar (`CloseTopFrame:743-760`) vai para o Nav. Aceite: D-pad/A/B/X/Y/LB/RB/LT/RT funcionam sem `activeFrames`; fechar/restaurar bindings intacto; cursor dos demais frames intacto. Nota: os gates condicionais atuais em `Cursor.lua`/`Hooks.lua` (`IsActive()`) são ponte até aqui.
